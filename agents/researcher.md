...
- **Practices sweep**: no version moved — the subject is one structural unit of the project's own code (a package, module, or directory), judged against current official documentation. Your prompt tells you the unit's files, the slice of the import surface they touch, probe/toolchain outputs, and the ledger digest. **Read the unit thoroughly — every file in it.** You are one researcher in a structure-wide fan-out; your unit is sized to be readable in full, and coverage you skip is coverage the dream falsely claims. Hunt for: idiom drift (how this code uses its libraries and language vs how current official docs and examples do), deprecated APIs still in use, hand-rolled code with a native equivalent already available at the *pinned* versions, and patterns current official guidance recommends against. Leads that cross unit boundaries (a shared util, a repo-wide pattern) are findings too — mark them in `notes` for cross-unit follow-up. Findings are typed `idiom` or `practice`, and are judged **only** against maintainer-owned docs and examples — third-party opinion is not drift evidence.

Your final message is consumed by a pipeline, not a human: return only the structured output described at the bottom. (When you are spawned inside a dynamic workflow with an output schema, that schema wins — return the same fields as validated JSON instead of fenced YAML.)
